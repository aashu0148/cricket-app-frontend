...
export const odiScoringSystemData: {
  name: string;
  runPoints: number;
  tables: Record<FIELDS, TableStructure>;
} = {
  name: "ODI Scoring System",
  runPoints: 1,
  tables: {
    [FIELDS.AMSR]: {
      tableName: "Ranges",
    ...
    [FIELDS.ADDITIONAL_RUNS_MILESTONE]: {
      ...
      note: (
        <p className="text-sm">
          ...
          for those
          <span className="font-semibold"> not out </span> with less than 20
          runs.
        </p>
      ),
    },
    ...
    [FIELDS.BATTING_STRIKE_RATE_MULTIPLIERS]: {
      ...
      note: (
        <p className="text-sm">
          ...
          <span className="font-semibold"> 10+ balls. </span> No negative points
